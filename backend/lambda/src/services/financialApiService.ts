...
export class FinancialApiService {
  ...
  try {
    ...
    const range = periodMap[period.toLowerCase()] || '1y';
    ...
    if (!data || !data.chart || !data.chart.result || data.chart.result.length === 0) {
      logger.error(`Invalid Yahoo Finance response for ${symbol}:`, data);
    ...
    if (!result.timestamp || !result.indicators || !result.indicators.quote || !result.indicators.quote[0]) {
      logger.error(`Missing data in Yahoo Finance response for ${symbol}`);
    ...
    // Check if quotes have data
    if (!quotes.close || quotes.close.length === 0) {
      logger.error(`No price data in Yahoo Finance response for ${symbol}`);
    ...
    for (let i = 0; i < timestamps.length; i++) {
      ...
      timeSeriesData[date] = {
        '1. open': (quotes.open[i] || quotes.close[i]).toFixed(4),
        '2. high': (quotes.high[i] || quotes.close[i]).toFixed(4),
        '3. low': (quotes.low[i] || quotes.close[i]).toFixed(4),
        '4. close': quotes.close[i].toFixed(4),
        '5. volume': (quotes.volume[i] || 0).toString()
      };
    }
    ...
    if (dataPoints === 0) {
      logger.error(`No valid data points for ${symbol} from Yahoo Finance`);
      throw new Error('No valid price data extracted');
    }
    
    logger.info(`Successfully fetched ${dataPoints} days from Yahoo Finance for ${symbol}`);
    
    // Get the latest price from meta data if available
    const currentPrice = result.meta?.regularMarketPrice || quotes.close[quotes.close.length - 1];
    const previousClose = result.meta?.previousClose || quotes.close[quotes.close.length - 2];
    ...
    return {
      'Meta Data': {
        ...
        '5. Time Zone': result.meta?.timezone || 'US/Eastern',
        '6. Current Price': currentPrice,
        '7. Previous Close': previousClose
      },
    ...
    // If Yahoo fails, try to fall back to Finnhub for current data at least
    if (FINNHUB_API_KEY) {
      logger.info(`Falling back to Finnhub for ${symbol}`);
      try {
        const quote = await this.getStockQuote(symbol);
        
        if (quote && quote.c) {
          // Create minimal time series with current data
          const today = new Date().toISOString().split('T')[0];
          const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
          
          return {
            'Meta Data': {
              '1. Information': 'Daily Prices (Finnhub Fallback)',
              '2. Symbol': symbol,
              '3. Last Refreshed': today,
              '4. Output Size': 'Compact',
              '5. Time Zone': 'US/Eastern',
              '6. Current Price': quote.c,
              '7. Previous Close': quote.pc
            },
            'Time Series (Daily)': {
              [today]: {
                '1. open': (quote.o || quote.c).toString(),
                '2. high': (quote.h || quote.c).toString(),
                '3. low': (quote.l || quote.c).toString(),
                '4. close': quote.c.toString(),
                '5. volume': (quote.v || 0).toString()
              },
              [yesterday]: {
                '1. open': quote.pc.toString(),
                '2. high': quote.pc.toString(),
                '3. low': quote.pc.toString(),
                '4. close': quote.pc.toString(),
                '5. volume': '0'
              }
            }
          };
        ...
        logger.error(`Finnhub fallback also failed for ${symbol}:`, finnhubError);
      }
    }
    
    // If everything fails, throw the original error
    throw new Error(`Unable to fetch data for ${symbol}: ${error.message}`);
  }
}

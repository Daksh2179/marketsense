...
`;
      ...
      return "I'm having trouble processing your question right now. Please try asking something else about markets or investments!";
    }
  }
  ...
  private static createFallbackNewsAnalysis(headlines: string[]) {
    return {
      positiveSummary: ['Market activity showing continued interest', 'Company maintaining operational momentum'],
      negativeSummary: ['Some market volatility concerns persist'],
      overallSentiment: 0.1,
      keyThemes: [
        { theme: 'Market Activity', sentiment: 0.2 },
        { theme: 'Operations', sentiment: 0.1 }
      ],
      marketImpact: 'Mixed signals suggest neutral to cautious optimism in near term'
    };
  }
  ...
  private static createFallbackPortfolioAnalysis(
    ...
    const profiles = {
      conservative: {
        overallHealth: 'Moderate - Focus on stability and risk management',
        strengths: ['Quality large-cap holdings', 'Established market leaders'],
        weaknesses: ['Consider more defensive sectors', 'Monitor volatility exposure'],
        suggestions: ['Add dividend stocks for income', 'Reduce concentration risk'],
        riskAssessment: 'Medium risk - suitable for conservative approach'
      },
      neutral: {
        ...
        weaknesses: ['Room for international exposure', 'Consider value opportunities'],
      ...
      aggressive: {
        overallHealth: 'Growth-focused - High potential with increased volatility',
        strengths: ['Strong growth sector exposure', 'Momentum positioning'],
        weaknesses: ['High correlation during downturns', 'Missing small-cap exposure'],
        suggestions: ['Add high-growth opportunities', 'Consider options strategies'],
        riskAssessment: 'High risk/reward with significant volatility'
      }
    };

    return {
      ...profiles[riskTolerance],
      diversificationScore: Math.min(8, sectors.length * 2.5)
    };
  }
  ...
  private static createFallbackComparisonInsights(companies: any[]) {
    return [
      ...
      'Consider diversification benefits of mixed selections'
    ];
  }
